...
function BaggyJeans() {
  const { jens, setjens } = useContext(ProductContext);
  return (
    <div>
      <h1 className="bg-gray-100 px-4 py-2 mt-20 text-xl font-mono flex items-center gap-2">
        Jens <span className="text-sm text-gray-600">({jens.length} items)</span>
      </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="col-span-1 md:col-span-4 flex flex-col md:flex-row gap-6 items-stretch mt-10">
            ...
            <div className="md:flex-[2] flex-1 h-64 sm:h-80 md:h-[850px]">
              <img
                src="https://i.pinimg.com/736x/e5/19/05/e51905218001bdb106ebb7a026626bdf.jpg"
                alt="Special"
                className="w-full h-full object-cover"
              />
            </div>
            ...
            <div className="md:flex-1 flex flex-col gap-4 sm:gap-6 mt-4 md:mt-0">
              {jens.slice(8, 10).map((item, index) => (
                <div
                  key={index}
                  onClick={() => setbag(item)}
                  className="bg-white overflow-hidden hover:scale-105 transform transition duration-300 ease-in-out mx-auto w-[90%] sm:w-full"
          ...
          {/* Next 8 cards */}
          {jens.slice(10, 18).map((item, index) => (
            <div
              key={index}
              onClick={() => setbag(item)}
              className="bg-white overflow-hidden hover:scale-105 transform transition duration-300 ease-in-out mx-auto w-[90%] sm:w-full"
